...
const els = {
  ...
  prodSku: $('#prod-sku'),
  prodName: $('#prod-name'),
  prodPrice: $('#prod-price'),    // SELL PRICE
  prodCost: $('#prod-cost'),      // read-only (dari pembelian)
  prodStock: $('#prod-stock'),    // read-only (dari pembelian)
  ...
  // Scanner reusable (POS + Product Manager)
  scanBtn: $('#scan-btn'),                    // tombol scan di POS
  scanModal: $('#scanModal'),
  scanVideo: $('#scanVideo'),
...
window.addEventListener('DOMContentLoaded', () => {
  // silent login kalau server pakai session PIN
...
async function loadProducts(){
  try{
    ...
    PRODUCTS = (j.data || []).map(p => ({
      ...
      barcode: p.barcode || '' // pastikan ada field kalau backend sudah support
    }));
...
// ---------------- Scanner (reusable: POS & Product barcode input) ----------------
let scanModalInst = null;
let codeReader = null;
let currentDeviceId = null;
let activeStream = null;
// target: 'pos' atau element INPUT (untuk prodBarcode)
let scanTarget = 'pos';
...
function setupScanner(){
  ...
  // tombol scan di form produk → isi field barcode
...
async function startDecodeFromDevice(deviceId){
  ...
  codeReader.decodeFromVideoDevice(deviceId || null, els.scanVideo, (result, err) => {
    ...
    // NotFoundError per frame → abaikan
  });
}
